...
class U1652DatasetEval(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transforms is not None:
            img = self.transforms(image=img, random_state=random_state)['image']

        label = int(sample_id)
        if self.given_sample_ids is not None:
            if sample_id not in self.given_sample_ids:
                label = -1
        img_np = tensor_to_image(img.clone())
        if self.count <2:
            import os
            cv2.imwrite(f'{os.path.basename(img_path)}', img_np)
            print(os.path.abspath(f'{label}.jpg'))
            print(img_path)
...
def tensor_to_image(tensor, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    ...
    # 如果图像是用Normalize标准化的，则可能需要裁剪到[0, 1]范围
    img = np.clip(img, 0, 1)
    
    # 转换为uint8类型，并根据需要调整颜色通道（例如从RGB转BGR）
    img = (img * 255).astype(np.uint8)
    if img.shape[2] == 3:  # 检查是否为彩色图像
        img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)  # 根据你的需要可能是cv2.COLOR_BGR2RGB
    
    return img
def get_transforms(img_size,
                   mean=[0.485, 0.456, 0.406],
                   std=[0.229, 0.224, 0.225],uncertain_type = 'None',p=1,strenth=(57, 59)):
    if uncertain_type == 'None':
        print(f'uncertain_type == {uncertain_type}')
        ...
        blur_limit=strenth#0.3*((50-1)*0.5-1)+0.8
        print(f'blur_limit=={blur_limit}','p=',p)
        val_transforms = A.Compose([A.Resize(img_size[0], img_size[1], interpolation=cv2.INTER_LINEAR_EXACT, p=1.0),
        ...
        val_transforms = A.Compose([A.Resize(img_size[0], img_size[1], interpolation=cv2.INTER_LINEAR_EXACT, p=1.0),
                                    A.RandomBrightnessContrast(
                                        brightness_limit=beta,  # 固定亮度调整
                                        contrast_limit=0,       # 无对比度变化
    ...
    elif uncertain_type == 'dropout':
        holes=strenth
        print(f'holes=={holes}','p=',p)
        val_transforms = A.Compose([A.Resize(img_size[0], img_size[1], interpolation=cv2.INTER_LINEAR_EXACT, p=1.0),
                                    A.CoarseDropout(num_holes_range=(holes,holes*2), hole_height_range=(int(0.1 * img_size[0]),int(0.2 * img_size[0])), hole_width_range=(int(0.1 * img_size[0]),int(0.2 * img_size[0])), p=p),
                                    A.Normalize(mean, std),
                                    ToTensorV2(),
                                    ])
