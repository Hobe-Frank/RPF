...
class DinoV2_self(nn.Module):
    ...
    def __init__(self, model_name: _DINO_V2_MODELS, layer1: int = 39,  facet1: _DINO_FACETS = "value", use_cls=False,
        ...
        if 'vitg14' in self.model_name:
            ...
            self.dino_model = torch.hub.load(r'D:\python_code\MixVPR(hgs)\models\backbones\facebookresearch_dinov2_main\dinov2', self.model_name, trust_repo=True, source='local')  # 加载DINOv2预训练模型/root/.cache/torch/hub/facebookresearch_dinov2_main
            ...
            self.dino_model = torch.hub.load('/root/.cache/torch/hub/facebookresearch_dinov2_main',self.model_name, trust_repo=True, source='local')  # 加载DINOv2预训练模型
